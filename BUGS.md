...
## BUG-8: Mutable class-level list defaults in `TerminalTextView`

**Severity:** Low (latent)
**Files:** `pagemark/view.py:418-419`

```python
class TerminalTextView(TextView):
    lines: list[str] = []
    line_styles: list[list[int]] = []
```

These mutable defaults are shared across all instances until shadowed by instance attributes (which happens in `render()`). If multiple `TerminalTextView` instances existed before their first render, they would share and mutate the same lists.

**Fix:** Initialize in `__init__` or use `field(default_factory=list)`.

---

## BUG-9: Unreachable dead code in `_build_output_list`

**Severity:** Cosmetic
**Files:** `pagemark/print_dialog.py:111-113`
...
The `if not options` check is unreachable because `"PDF File"` was unconditionally appended on the line above.

**Fix:** Remove the dead `if not options` block.
